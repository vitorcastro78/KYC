    ...
    ILogger<AdverseMediaService> log) : IAdverseMediaService
{
    ...
    public async Task<AdverseMediaResult> ScanAsync(
        string entityName,
        string? nif = null,
        int lookbackYears = 2,
        CancellationToken ct = default)
    {
        ...
        var cutoff = DateTime.UtcNow.AddYears(-Math.Clamp(lookbackYears, 1, 10));
        var filtered = hits.Where(h => h.PublishedAt is null || h.PublishedAt >= cutoff).ToList();
        return new AdverseMediaResult(filtered);
    }
