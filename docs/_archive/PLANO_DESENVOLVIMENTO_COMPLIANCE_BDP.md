...
> **Base:** `docs/CHECKLIST_HOMOLOGACAO_BDP.md` + `docs/MATRIZ_REQUISITOS_INSTITUCIONAIS.md`  
> **Estado actual (Maio 2026):** ~95% código produto (ver §2.3 deste plano); contingências manuais ✅; pendente **execução** homologação (E2E 10 cenários, dossier, pen test) e credenciais X1–X4.  
...
| D-13 | ContextMemory-only LLM, OFAC SLS download | `KycLlmEngine`, Workers |
...
Resumo: credenciais externas X1–X6, homologação E10 (E2E, dossier, pen test), ContextMemory/Blob (fase 2).
...
| E6-03 | `AmlComplianceReportService`: `aiModelsJson` só ContextMemory | P1 | `AmlComplianceReportService.cs` | Sem cloud |
...
# Base
KYC_DB_CONNECTION=...
CONTEXT_MEMORY_BASE_URL=http://host:11434
...
| ContextMemory indisponível | Score heurístico + relatório template (já existe fallback) |
