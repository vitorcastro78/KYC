# Plataforma KYC AI: Blueprint de IA de Cursor

> **Pila:** .NET 9 · Servidor Blazor · PostgreSQL 16 + pgvector · Azure Service Bus · Kernel semántico · Ollama (Qwen3.5:9b) · API de Anthropic (Claude Sonnet)  
> **Autentificación:** Microsoft Entra ID (OIDC)  
> **Infraestructura:** Azure App Service · Azure Container Registry · Azure Key Vault  
> **Versión:** 1.1 · Mayo 2026

---

## Cómo utilizar este documento con Cursor

1. Coloque este archivo en la raíz del repositorio como `BLUEPRINT.md`.
2. Abra el Cursor e inicie una conversación con `@BLUEPRINT.md`
3. Utilice las indicaciones de cada sección para guiar el cursor paso a paso.
4. El Cursor debe seguir la Arquitectura Limpia que se describe a continuación.

---

## 1. Descripción general del sistema

La plataforma KYC AI evalúa el riesgo crediticio corporativo analizando automáticamente:

- La **entidad prestataria** (empresa que solicita el crédito)
- Todos los **socios, administradores y UBO** (titulares reales) hasta N niveles
- Cruce con **listas de sanciones, PEP, medios adversos, datos judiciales y financieros**
- **Ingestión de documentos de entidad** (PDF, DOCX, imágenes) con extracción estructurada a BD
- Generación de **reporte narrativo de riesgo** vía LLM (Claude API + Qwen3.5 local)
...
```
### Flujo de ingesta de documentos (implementado)
```
...
## 2. Arquitectura: Arquitectura limpia
```
src/
...
- Los trabajadores dependen de `Application`
...
```
### 3.2 Entidades del documento (persistencia estructurada en la BD)

Los datos extraídos de los documentos **no permanecen en un blob JSON**; siguen el mismo principio que los datos de las API (`RiskSignal`, `CaseParty`): tablas relacionales consultables.
```csharp
...
```
**Paridad de API frente a documento:**

| Origen | Tabla de objetivos | Documento equivalente |
|--------|----------------|----------------------|
| OpenSanctions, NewsAPI, AT | `risk_signals` | `DocumentConsistencyChecker` → `risk_signals` (`Source = "Document:{id}"`) |
| GLEIF | `case_parties` | `document_extracted_parties` (indicado en el documento) |
| En deudores | `risk_signals` (Financiero) | hecho `Nif` cruzado con el índice local AT |

---

## 4. Capa de aplicación: casos de uso e interfaces

### 4.1 Interfaces de infraestructura (definidas en Aplicación, implementadas en Infraestructura)
```csharp
...
```
### 4.2 Casos de uso (CQRS con MediatR)
```csharp
// Comandos
...
### 5.1 Base de datos: EF Core + PostgreSQL + pgvector
```csharp
...
```
### 5.5 Ingestión de documentos: canalización asincrónica
```csharp
...
```
**Puntos finales de API (KYC.Web/Program.cs):**
```
...
```
**Integración con el canal KYC:**
- Después de la ingestión `Completed` → `PublishCaseRescreenAsync` (reevaluación automática)
- `KycCasePipelineRunner.BuildContext` lee `DeclaredFacts` / `DeclaredParties` de BD
- `DocumentConsistencyChecker` se ejecuta antes de la puntuación LLM y genera señales `Inconsistency`
- Auditoría: `DocumentUploaded`, `DocumentExtracted` (con `LlmPromptHash`)

**Limitaciones de MVP:**
- Los archivos PDF escaneados sin texto seleccionable fallan: use JPEG/PNG o PDF nativo
- Almacenamiento local (`Data/cases/`): Azure Blob está en la fase 2
- Incrustaciones por documento (RAG pgvector) — fase 2

### Motor 5.2 LLM: núcleo semántico + LLM dual
```csharp
...
```
### 5.3 Azure Service Bus: orquestación asincrónica
```csharp
...
```
### 5.4 Fuentes de datos externas: clientes HTTP escritos
```csharp
...
```
### 6.2 Actualizaciones en tiempo real: Blazor + SignalR
```csharp
...
```
### 6.3 Autenticación: Microsoft Entra ID
```csharp
// Program.cs
...
### 7.1 Azure Key Vault: todos los secretos aquí, nunca en appsettings
```csharp
// Program.cs
...
```
### 7.2 RGPD: Políticas de retención de datos
```csharp
...
```
### 7.3 Seguimiento de auditoría: mensaje auditable
```csharp
...
```
### Fase 2: motor KYC principal (Sprint 3 a 5)
```
...
```
### Fase 3: IA e informes (Sprint 6–7)
```
...
```
### Fase 4: interfaz de usuario y flujo de trabajo (Sprint 8–9)
```
...
```
### Fase 5: Fuentes adicionales y cumplimiento (Sprint 10-12)
```
...
```
### Fase 5b: ingesta de documentos (implementada · mayo de 2026)
```
...
## 10. Indicaciones del cursor

### Mensaje de inicio (para empezar desde cero)
```
@BLUEPRINT.md
...
## 11. Variables de entorno requeridas (ubicación de desarrollo)
```bash
...
## 12. .cursorrules (colocar en la raíz del repositorio)
```
...
*Este documento es el principal artefacto de desarrollo. Todas las decisiones sobre arquitectura, pila y cumplimiento se basan aquí. Versión 1.1: mayo de 2026.*