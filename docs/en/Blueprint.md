...
## How to use this document with the Cursor

1. Place this file in the root of the repository as `BLUEPRINT.md`
2. Open the Cursor and start a conversation with `@BLUEPRINT.md`
3. Use the prompts in each section to guide the Cursor step by step
4. The Cursor must follow the Clean Architecture described below

---

## 1. System Overview

The KYC AI platform assesses corporate credit risk by automatically analyzing:

- The **borrowing entity** (company requesting credit)
- All **shareholders, directors and UBOs** (Beneficial Owners) up to N levels
- Cross-reference with **sanctions lists, PEP, adverse media, judicial and financial data**
- **Ingestion of entity documents** (PDF, DOCX, images) with structured extraction to DB
- Generation of **narrative risk report** via LLM (Claude API + local Qwen3.5)
...
### Document ingestion flow (implemented)
...
- `Web` depends on `Application` (never on `Infrastructure` directly)
- Workers depend on `Application`
...
### 3.2 Document entities (structured persistence in the DB)

Data extracted from documents **does not remain in a JSON blob** — follows the same principle as API data (`RiskSignal`, `CaseParty`): queryable relational tables.
...
**API parity vs. Document:**

| Origin | Target table | Document equivalent |
|--------|----------------|----------------------|
| OpenSanctions, NewsAPI, AT | `risk_signals` | `DocumentConsistencyChecker` → `risk_signals` (`Source = "Document:{id}"`) |
| GLEIF | `case_parties` | `document_extracted_parties` (stated in doc) |
| AT debtors | `risk_signals` (Financial) | fact `Nif` crossed with AT local index |

---

## 4. Application Layer — Use Cases & Interfaces

### 4.1 Infra interfaces (defined in Application, implemented in Infrastructure)
...
### 4.2 Use Cases (CQRS with MediatR)
...
### 5.5 Document ingestion — asynchronous pipeline
...
- After ingestion `Completed` → `PublishCaseRescreenAsync` (automatic re-screening)
- `KycCasePipelineRunner.BuildContext` reads `DeclaredFacts` / `DeclaredParties` from BD
- `DocumentConsistencyChecker` executes before LLM scoring and generates signals `Inconsistency`
- Audit: `DocumentUploaded`, `DocumentExtracted` (with `LlmPromptHash`)

**MVP Limitations:**
- Scanned PDFs without selectable text fail — use JPEG/PNG or native PDF
- Local storage (`Data/cases/`) — Azure Blob goes to phase 2
- Embeddings per document (RAG pgvector) — phase 2
...
### 5.3 Azure Service Bus — Asynchronous orchestration
...
### 5.4 External data sources — Typed HTTP Clients
...
### 7.1 Azure Key Vault — all secrets here, never in appsettings
...
### 7.3 Audit trail — Auditable prompt
...
### Phase 2 — Core KYC Engine (Sprint 3–5)
...
### Phase 3 — AI & Reporting (Sprint 6–7)
...
### Phase 4 — UI & Workflow (Sprint 8–9)
...
### Phase 5 — Additional Sources & Compliance (Sprint 10–12)
...
### Phase 5b — Document ingestion (implemented · May 2026)
...
### Initial prompt (to start from scratch)
...
## 11. Required environment variables (dev location)
...
## 12. .cursorrules (put in the root of the repository)
...
*This document is the main development artifact. All architecture, stack and compliance decisions are based here. Version 1.1 — May 2026.*
